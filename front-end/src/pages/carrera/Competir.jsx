...
const Competir = ({  idCircuito, onDriverClick }) => {
  ...
  useEffect(() => {
    const cargarResultados = async () => {
      ...
      const API_URL = `https://api.jolpi.ca/ergast/f1/2024/${id}/results.json`;
  ...
  const top3Drivers = [p2, p1,].filter(Boolean);
  const remainingDrivers = drivers.slice(3);

  return (
    <section className="f1-results-section">
    {loading ? (
        <p className="loading-message">Cargando resultados de los pilotos...</p>
      ) : drivers.length > 0 ? (
        <>
          <div className="top3-grid">
            {top3Drivers.map((driver) => (
              <div
                className={`top-driver-card card-pos-${driver.position}`}
                key={driver.id}
        ...
        <p className="no-data-message">No se encontraron resultados para esta carrera.</p>
      )}
    </section>
  );
};

export default Competir;